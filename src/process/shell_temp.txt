...
Process *addJob(Process *head, Process *newProcess){
...
void penn_shredder(char* buffer){
    ...
    if(strcmp("bg", cmd -> commands[0][0]) == 0){
        ...
        if(cmd -> commands[0][1] != NULL){
            ...
            Process *bgJob = getJob(head, job_id);
    ...
    if(strcmp("fg", cmd -> commands[0][0]) == 0){
        ...
        if(cmd -> commands[0][1] != NULL){
            int job_id = atoi(cmd -> commands[0][1]);
            Process *fgJob = getJob(head, job_id);
    ...
    Process *newProcess = NULL; // create a new job each time penn shredder is run
